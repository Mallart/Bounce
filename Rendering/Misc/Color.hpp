...
namespace Bounce::Render
{
	...
	protected:
		uint8_t CMax() const { return R() > G() ? (R() > B() ? R() : (G() > B() ? G() : B())) : (G() > B() ? G() : B()); }
		uint8_t CMin() const { return R() < G() ? (R() < B() ? R() : (G() < B() ? G() : B())) : (G() < B() ? G() : B()); }
	public:
